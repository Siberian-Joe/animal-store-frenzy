...
namespace Game.World.Features.ShelfRestocker
{
    ...
    public sealed class ShelfRestockerPart : EntityComponent,
        IShelfRestockerFeature,
        IRestockSourceContract,
        IInteractionSourceRoleOwner
    {
        ...
        public int AvailableQuantity => Container.AvailableQuantity;

        private void Awake() => EnsureDependencies();

        private void OnValidate() => AutoAssignDependencies();

        public int RemoveUpTo(int amount) => Container.RemoveUpTo(amount);
        ...
        private ProductContainerPart Container =>
            _container != null
                ...
                : throw new System.InvalidOperationException(
                    $"{nameof(ShelfRestockerPart)} on '{name}' requires {nameof(ProductContainerPart)}.");
